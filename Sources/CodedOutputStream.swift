...
public class CodedOutputStream
{
    ...
    public func writeRawVarint32(let value:Int32) throws {
        var v = value
        while (true) {
            if ((v & ~0x7F) == 0) {
                try writeRawByte(byte:UInt8(v))
                break
            } else
            {
                try writeRawByte(byte: UInt8((v & 0x7F) | 0x80))
                v = WireFormat.logicalRightShift32(value:v,spaces: 7)
            }
        }
    }

    public func writeRawVarint64(let value:Int64) throws {
        var v = value
        while (true) {
            if ((v & ~0x7F) == 0) {
                try writeRawByte(byte:UInt8(v))
                ...
                try writeRawByte(byte: UInt8((v & 0x7F) | 0x80))
                v = WireFormat.logicalRightShift64(value:v, spaces: 7)
            }
        }
    }
}
